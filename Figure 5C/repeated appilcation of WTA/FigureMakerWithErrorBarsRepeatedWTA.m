...
% Plot scatter points for each q in descending order
for j = numel(unique_q):-1:1
    ...
    if any(idx)
        h = scatter(data_filtered(idx, 4), data_filtered(idx, 5), 15, 'MarkerFaceColor', 'none', ...
        'LineWidth', 1.5, 'MarkerEdgeColor', color_map(j, :));
        scatter_handles = [scatter_handles; h];
    end
end
...
legend(scatter_handles, legend_entries, 'Location', 'northwest');
xlabel('Repetitions');
...
saveas(gcf, 'ReadoutAccuracy_vs_Repetition_M8', 'jpg');
saveas(gcf, 'ReadoutAccuracy_vs_Repetition_M8', 'fig');
exportgraphics(gcf, 'ReadoutAccuracy_vs_Repetition_M8.pdf', 'ContentType', 'vector', 'Resolution', 600);
